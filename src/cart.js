...
document.addEventListener("DOMContentLoaded", () => {
    // טוען את המשתמש והעגלה מה-localStorage
    currentUser = JSON.parse(localStorage.getItem("currentUser"));
    basket = JSON.parse(localStorage.getItem("data")) || [];

    // עדכון הממשק בהתאם למצב המשתמש
    updateUserInterface();

    // עדכון הנתונים של העגלה לאחר טעינת הדף
    calculation();
    generateCartItems();
});
// עדכון פרטי הסל
...
document.getElementById("checkoutBtn").addEventListener("click", () => {
    // אם לא מחובר, לא נבצע הפניה לדף אחר, רק נבצע בדיקה אם יש עגלה
    if (!currentUser) {
        alert("אתה לא מחובר. תוכל להמשיך לסל הקניות, אך תצטרך להתחבר כדי לבצע הזמנה.");
        // לא מבוצעת הפניה לדף אחר, נשארים בדף הנוכחי של סל הקניות
    } else {
        console.log("ההזמנה מתבצעת");
        // כאן תוכל להוסיף קוד לשמירה או שליחה של ההזמנה
    }
});
function updateUserInterface() {
    ...
    if (currentUser) {
        ...
        loginButton.textContent = `שלום, ${currentUser.username}`; // הצגת שם המשתמש
        document.getElementById("userGreeting").textContent = `שלום, ${currentUser.username}`; // הצגת שם המשתמש בדף הראשי
    } else {
        ...
        document.getElementById("userGreeting").textContent = ''; // מנקה את שם המשתמש לאחר התנתקות
    }
}
