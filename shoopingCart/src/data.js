...
export class Product {
    ...
    static updateProduct(id, updatedFields) {
        ...
        if (!product) {
            // הצגת הודעה שהמוצר לא נמצא
            notification.textContent = `מוצר עם מזהה ${id} לא נמצא.`;
            notification.style.backgroundColor = "#f44336";  // צבע רקע אדום לשגיאה
        ...
        // הצגת הודעה שהמידע עודכן בהצלחה
        notification.textContent = `המידע של המוצר עם מזהה ${id} עודכן בהצלחה.`;
        notification.style.backgroundColor = "#4CAF50";  // צבע רקע ירוק להצלחה
        notification.style.display = "block";
